...
目录是完整建设包的参考索引，可按任务合并文件、复用已有产物。完整候选包不要求真实上线；发布与运行目录可交付方案和待满足的准入条件。单阶段任务只生成当前 Skill 负责的成果及必要上游引用，不为凑目录生成空文件。
...
- 缺少源数据或权限：保留映射要求和缺口；可生成明确标记为合成、与真实实例隔离的技术验证数据，不冒充源系统记录。
